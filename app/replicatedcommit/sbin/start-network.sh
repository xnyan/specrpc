...
for c in ${client_machine[@]}; do
    ssh -t $c "sudo -b iftop -t -L 0 > $dir/network.log" 
    echo "Start network measurement at client" $c
done

for s in ${server_machine[@]}; do
    ssh -t $s "sudo -b iftop -t -L 0 > $dir/network.log" 
    echo "Start network measurement at server" $s
done

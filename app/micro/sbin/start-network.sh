...
for c in "${CONFIG_LIST[@]}"; do
  ...
  mId=${c[0]}
  ip=${c[2]}
  ssh -t $ip "sudo -b iftop -t -L 0 > $dir/client-$mId-net.log" 
  echo "Start network measurement at client" $mId $ip
done
...
for s in "${CONFIG_LIST[@]}"; do
  ...
  sId=${s[0]}
  ip=${s[1]}
  ssh -t $ip "sudo -b iftop -t -L 0 > $dir/server-$sId-net.log" 
  echo "Start network measurement at server" $sId $ip
done
